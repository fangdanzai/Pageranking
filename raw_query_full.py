...
def create_output(result, train_queries, test_queries):
        ...
        to_ranklib(result_train, "../data/output/raw_query_full_train.txt")

        tset = set(test_queries["queryId"].values)
        result_test = result.loc[result["queryId"].isin(tset)]
        to_ranklib(result_test, "../data/output/raw_query_full_test.txt")
        result_test[["queryId","itemId"]].to_csv("../data/output/raw_query_full_test_ids.txt", index=False)
...
def to_ranklib(df, filename):
    ...
    for row in df[["queryId","itemId","clicked","viewed","purchased", "itemView","itemClick",
        "itemPurchase", "pricelog2", "jaccard", "maxp","minp","medianp","meanp","rangep", "avg_duration",
        "productNameSize", "querySize", "queriesPerSession", "durationPerSession"]].itertuples():
